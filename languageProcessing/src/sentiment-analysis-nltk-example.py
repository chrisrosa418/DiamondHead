...
def get_words_in_tweets(tweets):
    ...
    #print 'these are all_words : %s' % (all_words, )
    return all_words
...
def get_word_features(wordlist):
    ...
    #print 'these are word_features : %s' % (word_features, )
...
def extract_features(document):
    ...
    #print 'these are features : %s' % (features, )
    return features
...
runtweets.append('Thank')  # test tweet incase
poscount = 0
negcount = 0
print 'runtweets : %s' % (runtweets, )
for tweett in runtweets:
  valued = classifier.classify(extract_features(tweett.split()))
  print 'valued : %s' % (valued, )
  print (valued)
  if valued == 'negative':
    negcount = negcount + 1
  else:
